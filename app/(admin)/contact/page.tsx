...
interface Contact {
  ...
  tags: string[];
  createdAt: string | Date; // Allow for Firestore Timestamp or string
}
...
const ContactsPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribeContacts = onSnapshot(contactsRef, (snapshot) => {
      const contactsData = snapshot.docs.map((doc) => ({
        ...
        isSubscriber: false, // Non-subscribers
        tags: doc.data().tags || [],
        createdAt: doc.data().createdAt?.toDate?.() || doc.data().createdAt || new Date(),
      })) as Contact[];

      // Listener for subscribers collection
      ...
      const unsubscribeSubscribers = onSnapshot(subscribersRef, (snapshot) => {
        const subscribersData = snapshot.docs.map((doc) => ({
          ...
          email: doc.data().email || "",
          phone: doc.data().phone || "",
          isSubscriber: true, // Subscribers
          tags: doc.data().tags || [],
          createdAt: doc.data().createdAt?.toDate?.() || doc.data().createdAt || new Date(),
        })) as Contact[];

        // Combine and set contacts
        const combinedContacts = [...contactsData, ...subscribersData];
        setContacts(combinedContacts);
        setFilteredContacts(combinedContacts);
      });

      // Cleanup subscribers listener
      return () => unsubscribeSubscribers();
    });

    // Cleanup contacts listener
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      const contactData = {
        ...
        tags: formData.tags,
        createdAt: selectedContact ? formData.createdAt : now,
      };

      try {
        if (selectedContact) {
          // Update existing contact
          const docRef = doc(db, collectionName, selectedContact.id);
          await updateDoc(docRef, contactData);
        } else {
          // Add new contact
          await addDoc(collection(db, collectionName), contactData);
        }
        closeForm();
      } catch (error) {
  ...
  const deleteContact = async (id: string) => {
    try {
      ...
      if (!contact) return;
      const collectionName = contact.isSubscriber ? "subscribers" : "contacts";
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {formData.tags.map((tag) => (
                      <div key={tag} className={`px-2 py-1 text-sm rounded-full flex items-center ${darkMode ? "bg-gray-700 text-gray-300" : "bg-gray-200 text-gray-700"}`}>
                        {tag}
                        <button type="button" onClick={() => removeTag(tag)} className="ml-1 text-gray-400 hover:text-gray-200">
                          <X size={14} />
                        </button>
                      </div>
                    ))}
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button type="button" onClick={addTag} className="px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md">
                      Add
                    </button>
                  </div>
                </div>
