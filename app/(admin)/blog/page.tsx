...
type BlogPost = {
  ...
  videoUrl?: string;
  featuredImagePublicId?: string; // Added for Cloudinary
};
...
export default function BlogManagement() {
  ...
  const confirmDelete = async () => {
    if (postToDelete !== null) {
      try {
        // Find the post to get its featuredImagePublicId
        const post = posts.find((p) => p.id === postToDelete);
        if (post?.featuredImagePublicId) {
          await fetch('/api/cloudinary/delete', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ publicId: post.featuredImagePublicId }),
          });
        }
  ...
  const handleEditPost = (id: string) => {
    router.push(`/blog/editPost?id=${id}`);
  };
